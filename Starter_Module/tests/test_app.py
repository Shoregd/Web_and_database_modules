...
And: I send "Greg,Alice,Terry,Xaviar,Alex"
Then: I should get a 200 response with the names organised by alphabetical order I.e (Alex,Alice,Greg,Terry,Xaviar)
'''
def test_post_sort_names_listed_names(web_client):
    response = web_client.post('/sort_names', data={'names':'Greg,Alice,Terry,Xaviar,Alex'})
    assert response.status_code == 200
    assert response.data.decode('utf-8') == 'Alex,Alice,Greg,Terry,Xaviar'
...
And: I send '?add=Eddie'
Then: I shoult get a 200 response with the name Eddie added to a pre-held list.
I.e Karim, Alice, Julia, Eddie
'''

def test_names_returns_name_added_to_list(web_client):
    response = web_client.get('/names?add=Eddie')

    assert response.status_code == 200
    assert response.data.decode('utf-8') == 'Alice, Eddie, Julia, Karim' 

def test_can_add_multiple_names_and_still_sorted(web_client):
    response = web_client.get('/names?add=Eddie,Terry,Greg')
    assert response.status_code == 200
    assert response.data.decode('utf-8') == 'Alice, Eddie, Greg, Julia, Karim, Terry' 

    
def test_error_when_names_not_seperated_by_commas(web_client):
    response = web_client.get('/names?add=Eddie,Terry:Greg')
    assert response.status_code == 200
    assert response.data.decode('utf-8') == 'Invalid name structure. Please seperate names by commas(,) I.e "name1,name2,name3"'